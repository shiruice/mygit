...
var entries = function () {
    ...
    for (var i = 0; i < entryFiles.length; i++) {
        if(entryFiles[i].indexOf(".js") == -1){
            continue;
        }
        var filePath = entryFiles[i];
        var arr = filePath.split("src/js/");
        arr.shift();
        var filename = arr[0].substring(0,arr[0].lastIndexOf('.'));
...
var html_plugins = function () {
    var entryHtml = glob.sync(srcDir + '/page/**')
    var r = []
    var entriesFiles = entries()
    for (var i = 0; i < entryHtml.length; i++) {
        if(entryHtml[i].indexOf(".ejs") == -1){
            continue;
        }
        var filePath = entryHtml[i];
        var arr = filePath.split("src/page/");
        arr.shift();
        var filename = arr[0].substring(0,arr[0].lastIndexOf('.'));
        // var filePath = entryHtml[i];
        // var filename = filePath.substring(filePath.lastIndexOf('\/') + 1, filePath.lastIndexOf('.'));
...
module.exports = function(options){
    ...
    var config = {
        entry: Object.assign(entries(), {
            ...
            'vendor': ['jquery','bootstrap','_','commonJs']
        }),
        output: {
